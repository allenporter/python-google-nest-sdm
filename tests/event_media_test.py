...
from google_nest_sdm.event_media import InMemoryEventMediaStore, Media, MediaTranscoder
...
async def test_transcoder(
    ...
    assert event_media.event_session_id == "DkY5Y3VKaTZwR3o4Y19YbTVfMF..."
    assert event_media.event_type == test_event_trait
    assert event_media.event_timestamp.isoformat(timespec="seconds") == ts.isoformat(
        timespec="seconds"
    )
